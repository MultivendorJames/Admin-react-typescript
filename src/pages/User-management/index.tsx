...
const Users = () => {
  ...
  const input = useDebounce(isEnteredItem, 1500);

  // Ftech data

  const getUsers = (isEnteredItem: string) => {
    const res = get(`/users?q=${isEnteredItem}`);
    return res;
  };

  const { isLoading, data, isError } = useQuery(["users", input], () =>
    getUsers(input)
  );
  ...
  useEffect(() => {
    ...
    if (!enteredDropDown) return;

    if (enteredDropDown === "verified") {
      setUsersData(data?.users?.filter((p: userData) => p.verified));
    } else {
      setUsersData(
        data?.users?.filter(
          (p: userData) => p.role.toLowerCase() === enteredDropDown
        )
      );
    }
  }, [enteredDropDown]);
  ...
  return (
    <div className="">
      {/* 
            <div>
                <h1 className="mb-7 text-black-2 font-semibold text-2xl">Mange Users</h1>
            </div> */}
          ...
          {!isLoading && (
            <div className="max-w-full overflow-x-auto">
              <table className="w-full table-auto">
                <thead>
                  <tr className="bg-[#F49907] text-left dark:bg-meta-4">
                ...
                <tbody>
                  {usersData.map((user: userData) => (
                    <tr key={user._id}>
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="inline-flex rounded-full bg-[#F49907]  py-1 px-3 text-sm font-medium text-white">
                          {user.role}
                        </p>
                        <p></p>
                      </td>
